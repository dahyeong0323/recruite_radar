...
def update_source_state(state_path: Path, source: str, *, success: bool, seen_ids: list[str] | None = None, newest_timestamp=None) -> dict[str, Any]:
    ...
    if success:
        ...
        source_state = source_state.model_copy(update={"last_success_at": datetime.now().astimezone(), "recent_ids": merged_ids, "newest_timestamp": newest_timestamp, "consecutive_failures": 0})
    else:
...
def health_state(state_path: Path, *, git_push_failures: int = 0, telegram_failures: int = 0, pending_classifications: int = 0) -> str:
    ...
    failures = [value.get("consecutive_failures", 0) for value in data.get("sources", {}).values()]
...
def write_health_note(radar_root: Path, *, state: str, source_rows: list[dict[str, Any]], notes: list[str] | None = None) -> None:
    lines = [
        "# Recruiting Radar Health",
        "",
        f"- State: **{state}**",
        f"- Checked at: {datetime.now().astimezone().isoformat()}",
        "",
