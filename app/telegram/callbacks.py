...
async def handle_callback(settings: Settings, telegram: TelegramClient, query: dict) -> None:
    ...
    if not settings.dry_run:
        async with GLOBAL_VAULT_LOCK:
            result = GitSync(settings.vault_root, branch=settings.branch, radar_relative_path=settings.vault_relative_path, dry_run=False).commit_and_push(f"radar: mark {job_id} {status}")
